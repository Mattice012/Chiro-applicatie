...
function getFilteredData() {
    return cachedFinances.filter(t => {
        ...
        const matchesMonth = filterState.month === 'all' || tMonth === filterState.month;

        // 3. Afdeling Filter
        let tDepts = t.afdelingen || (t.afdeling ? [t.afdeling] : ['Algemeen']);
        if (typeof tDepts === 'string') tDepts = [tDepts]; // Fail-safe
        const matchesDept = filterState.department === 'all' || tDepts.includes(filterState.department);
...
function renderOverviewHtml() {
    ...
    const deptOptions = AFDELINGEN_CONFIG.map(a => `<option value="${a.naam}">${a.naam}</option>`).join('') + `<option value="Algemeen">Algemeen</option>`;
...
function renderTransactionList(data) {
    if (data.length === 0) return `<div class="flex flex-col items-center justify-center h-64 opacity-50"><i data-lucide="search-x" class="w-12 h-12 text-gray-600 mb-3"></i><p class="text-gray-400">Geen transacties gevonden.</p></div>`;
    ...
    if (filterState.month === 'all') {
        ...
        return Object.keys(groups).sort().reverse().map(mKey => {
            ...
            const label = new Date(y, m - 1).toLocaleDateString('nl-BE', { month: 'long', year: 'numeric' });
            return `
            <div>
...
function renderTransactionRow(t) {
    ...
    const sign = isIncome ? '+' : '-';
    const depts = (t.afdelingen || [t.afdeling || 'Algemeen']).map(d => `<span class="px-1.5 py-0.5 rounded text-[10px] bg-gray-800 text-gray-300 border border-gray-700">${d}</span>`).join(' ');
...
function restoreFilterUI() {
    ...
    if(s) { s.value = filterState.search; s.focus(); }
...
window.deleteTransaction = async (id) => {
    if (!await askConfirmation("Verwijderen", "Wil je deze transactie definitief verwijderen?")) return;
...
window.exportFinances = () => {
    ...
    let csvContent = "Datum;Type;Bedrag;Categorie;Omschrijving;Afdelingen;Gebruiker\n";
    data.forEach(row => {
...
window.askConfirmation = (title, msg) => {
    return new Promise(resolve => {
        if(window.askConfirmationGlobal) return window.askConfirmationGlobal(title, msg).then(resolve);
...
function renderBudgetsView(container) {
    ...
    const deptBalances = {};
    AFDELINGEN_CONFIG.forEach(a => deptBalances[a.naam] = { balance: 0, count: 0, color: a.kleur });
    ...
    const cards = [...AFDELINGEN_CONFIG, {naam: 'Algemeen', kleur: 'gray'}].map(afd => {
        const data = deptBalances[afd.naam] || {balance:0, count:0};
        const color = afd.kleur || 'gray';
        const isPos = data.balance >= 0;
        
        return `
        <div onclick="openBudgetDetail('${afd.naam}')" class="cursor-pointer group bg-[#181b25] border border-gray-800 p-6 rounded-2xl relative overflow-hidden hover:-translate-y-1 hover:shadow-2xl hover:shadow-${color}-500/10 transition-all duration-300">
            ...
            <div class="relative z-10">
                ...
                <div class="mt-4 pt-4 border-t border-gray-800 flex justify-between text-xs text-gray-500">
            ...
            <form onsubmit="handleTransactionSubmit(event)" class="space-y-8 relative z-10">
                ...
                <div class="space-y-3">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                        ${['Materiaal', 'Eten', 'Drank', 'Activiteit', 'Verhuur', 'Lidgeld', 'Kamp', 'Overige'].map(c => `
...
window.handleTransactionSubmit = async (e) => {
    ...
    btn.disabled = true; btn.innerHTML = '<div class="loader w-5 h-5 border-white mr-2"></div> Bezig...';
    ...
    // Checkboxes
    const checkboxes = document.querySelectorAll('#dept-checkboxes input:checked');
...
window.openBudgetDetail = (deptName) => {
    ...
    const total = rows.reduce((sum, t) => sum + (t.type === 'income' ? t.share : -t.share), 0);
